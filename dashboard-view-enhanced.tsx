...
import { exportAttendanceData } from "@/lib/exportService";
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
export function DashboardViewEnhanced({ showLoading, showToast }: DashboardViewProps) {
  ...
  const handleExport = async (format: string, options: ExportOptions) => {
    try {
      await exportAttendanceData(format, options);
      showToast(
        `Attendance data exported as ${format.toUpperCase()} successfully!`, 
        'success'
      );
    } catch (error) {
      ...
      <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Attendance Trend */}
            <ChartContainer title="Attendance Trend" onDownload={() => setIsExportDialogOpen(true)}>
            ...
            <ChartContainer title="Overall Attendance" onDownload={() => setIsExportDialogOpen(true)}>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={[
          ...
          {topAttendees.length > 0 && (
            <ChartContainer title="Top Attendees" onDownload={() => setIsExportDialogOpen(true)}>
        ...
        <TabsContent value="trends" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ChartContainer title="Attendance Patterns by Day & Time" onDownload={() => setIsExportDialogOpen(true)}>
            ...
            {/* Attendance by Site */}
            {siteData.length > 0 && (
              <ChartContainer title="Attendance by Site" onDownload={() => setIsExportDialogOpen(true)}>
                <ResponsiveContainer width="100%" height={300}>
          ...
          {/* Attendance by Type */}
          {typeData.length > 0 && (
            <ChartContainer title="Attendance by Person Type" onDownload={() => setIsExportDialogOpen(true)}>
              <ResponsiveContainer width="100%" height={300}>
        ...
        <TabsContent value="details" className="space-y-6">
          ...
          <ChartContainer title="Detailed Attendance Records" onDownload={() => setIsExportDialogOpen(true)}>
